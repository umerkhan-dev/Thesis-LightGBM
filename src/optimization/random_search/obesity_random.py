...
import time
from sklearn.model_selection import RandomizedSearchCV, RepeatedKFold, cross_validate,KFold
...
results.to_csv('obesity_random_ppr.txt', sep='\t', index=True)
...
output_file_path = os.path.join(os.getcwd(), 'obesity_random_per_fold_results.csv')
...
output_file_name = 'obesity_random_metrics_output.md'
...
plt.savefig('obesity_random_auc_roc_plot.png')
plt.show()
...
plt.savefig('obesity_CM_random_plot.png')
