...
import numpy as np
import time
from sklearn.model_selection import RandomizedSearchCV, RepeatedKFold, cross_validate,KFold,train_test_split
...
results.to_csv('dbean_random_ppr.txt', sep='\t', index=True)
...
output_file_path = os.path.join(os.getcwd(), 'dbean_random_per_fold_results.csv')
...
output_file_name = 'dbean_random_metrics_output.md'
...
plt.savefig('dbean_random_auc_roc_plot.png')
plt.show()
...
plt.savefig('cm_dbean_random_plot.png')
