...
from sklearn.metrics import auc,roc_curve,confusion_matrix,make_scorer,accuracy_score, f1_score,recall_score,precision_score,roc_auc_score,classification_report
import time
import numpy as np
import joblib


fp= "/Users/umarkhan/Downloads/abalone/abalone.data"
...
columns = [
    ...
    'Class'            # integer (but treated as continuous for age)
]
...
results.to_csv('abalone_grid_ppr.txt', sep='\t', index=True)
...
output_file_path = os.path.join(os.getcwd(), 'abalone_grid_per_fold_results.csv')
...
output_file_name = 'abalone_grid_metrics_output.md'
...
for i, color in zip(range(len(df['Class'].unique())), colors):
...
plt.savefig('abalone_grid_auc_roc_plot.png')
plt.show()
...
plt.savefig('cm_abalone_grid_plot.png')
