...
train_data_file = '~/statlog+landsat+satellite/sat.trn'
test_data_file = '~/statlog+landsat+satellite/sat.tst'
...
results.to_csv('statlog_grid_ppr.txt', sep='\t', index=True)
...
output_file_path = os.path.join(os.getcwd(), 'statlog_grid_per_fold_results.csv')
...
output_file_name = 'statlog_grid_metrics_output.md'
...
for i, color in zip(range(len(df_trn['Class'].unique())), colors):
...
plt.savefig('statlog_grid_auc_roc_plot.png')
plt.show()
...
plt.savefig('cm_statlog_grid_plot.png')
