...
df = pd.read_csv('~/oilspill/oilspill.csv')
df.info()    
le = LabelEncoder()
...
rkf = RepeatedKFold(n_splits=3, n_repeats=3,random_state=69)
...
model_filename = 'oilspill_model.joblib'
...
results.to_csv('Oil_spill_ppr.txt', sep='\t', index=True)
...
output_file_path = os.path.join(os.getcwd(), 'oilspill_grid_ppr.csv')
...
output_file_name = 'oilspill_grid_metrics_output.md'
...
plt.savefig('oilspill_grid_auc_roc_plot.png')
...
plt.savefig('oilspill_grid_cm_plot.png')
