...
def plot_class_distribution(data):
    ...
    data['Outcome'].value_counts().plot(kind='bar', color=colors)
    plt.title('Class Distribution for Diabetes')
    plt.xlabel('Class')
    plt.ylabel('Count')
    plt.xticks(rotation=360, ha='right')
    plt.savefig('/Users/umarkhan/Bayesian/datasets/target_class dist fig/Diabetes_target_dist.png')
    plt.show()
...
all_trials_df.to_csv("diabetes_optuna_all_trials.csv", index=False)

# Get the best model and parameters
best_params = study.best_params
best_model = lgb.LGBMClassifier(**best_params)
best_model.fit(X_train, y_train)
joblib.dump(best_model, 'diabetes_bst_optuna.joblib')

# Write the best parameters to a file
best_params_file = os.path.join(os.getcwd(), "diabetes_optuna_best_params.txt")
...
output_file_path = os.path.join(os.getcwd(), 'diabetes_optuna_per_fold_results.csv')
...
output_file_name = 'diabetes_optuna_metrics_output.md'
...
plt.savefig('diabetes_optuna_auc_roc_plot.png')
...
plt.savefig('diabetes_optuna_marketing_plot.png')
