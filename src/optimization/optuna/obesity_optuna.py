...
def plot_class_distribution(data):
    ...
    data['NObeyesdad'].value_counts().plot(kind='bar', color=colors)
    plt.title('Class Distribution for Obesity')
    plt.xlabel('Class')
    plt.ylabel('Count')
    plt.xticks(rotation=45, ha='right')
    plt.subplots_adjust(bottom=0.3)  # Adjust the value as needed
    plt.savefig('/Users/umarkhan/Bayesian/datasets/target_class dist fig/obesity_target_dist.png')
    plt.show()
...
# all_trials_df.to_csv("Obesity_optuna_all_trials.csv", index=False)

# # Get the best model and parameters
# best_params = study.best_params
# best_model = lgb.LGBMClassifier(**best_params, objective='multiclass', boosting_type='gbdt', metric='multi_logloss')
# best_model.fit(X_train, y_train)
# joblib.dump(best_model, 'Obesity_bst_optuna.joblib')

# # Write the best parameters to a file
# best_params_file = os.path.join(os.getcwd(), "Obesity_optuna_best_params.txt")
...
# output_file_path = os.path.join(os.getcwd(), 'Obesity_optuna_per_fold_results.csv')
...
# output_file_name = 'Obesity_optuna_metrics_output.md'
...
# plt.savefig('Obesity_optuna_auc_roc_plot.png')
...
# plt.savefig('Obesity_optuna_marketing_plot.png')
